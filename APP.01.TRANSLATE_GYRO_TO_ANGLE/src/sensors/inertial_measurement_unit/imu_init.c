...
const uint8_t gyro_scale_config  = GYRO_CONFIG_500;
...
uint32_t init_inertial_measurement_unit(void)
{
    ...
    /* Setup only full scale range */
    ret |= set_register_config(GYRO_CONFIG, gyro_scale_config);
    get_register_config(GYRO_CONFIG, &test);
    printf("GYRO_CONFIG expected = [%x] and get [%x]\n", gyro_scale_config , test);
