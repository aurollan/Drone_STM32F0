...
/**
 * @brief   Check if a communication error occured
 */
uint32_t i2c_error_occur(I2C_TypeDef* I2Cx)
{
    ...
    i2c_isr_err = 0;
    if (SET == I2C_GetFlagStatus(I2C1, I2C_FLAG_BERR))
    {
        i2c_isr_err |= I2C_FLAG_BERR;
    }
    if (SET == I2C_GetFlagStatus(I2C1, I2C_FLAG_BUSY))
    {
        i2c_isr_err |= I2C_FLAG_BUSY;
    }
    if (SET == I2C_GetFlagStatus(I2C1, I2C_FLAG_ARLO))
    {
        i2c_isr_err |= I2C_FLAG_ARLO;
    }
    if (SET == I2C_GetFlagStatus(I2C1, I2C_FLAG_OVR))
    {
...
uint32_t i2c_read(I2C_TypeDef* I2Cx, uint16_t register_to_read, uint8_t *received_data, uint8_t number_of_bytes_to_read)
{
    ...
    i2c_isr_err |= i2c_wait_flag_status(I2Cx, I2C_FLAG_TC, SET, max_retry);
    i2c_isr_err |= i2c_error_occur(I2Cx);
    if (!i2c_isr_err)
    {
        i2c_isr_err |= i2c_wait_flag_status(I2Cx, I2C_FLAG_RXNE, SET, max_retry);
        if (!i2c_isr_err)
        {
            ...
            i2c_isr_err = i2c_wait_flag_status(I2Cx, I2C_FLAG_STOPF, SET, max_retry);
            i2c_isr_err |= i2c_error_occur(I2Cx);
        }
    }
    return (i2c_isr_err);
