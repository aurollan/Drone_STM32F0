...
uint32_t i2c_read(uint16_t device_addr, uint16_t register_to_read, uint8_t *received_data, uint8_t number_of_bytes_to_read)
{
    ...
    index       = 0;
    i2c_isr_err_occured = 0;
    if (i2c_wait_flag_status(I2C1, I2C_FLAG_BUSY, RESET, I2C_MAX_RETRY) != 0) {
        return (I2C_FLAG_BUSY);
    }
    I2C_TransferHandling(I2C1, device_addr, 1, I2C_Generate_Start_Write, I2C_SoftEnd_Mode);
    i2c_isr_err_occured |= i2c_error_occur(I2C1);
    if (!i2c_isr_err_occured) 
    {
        i2c_isr_err_occured  = I2C_send_data_wait_and_check(I2C1, register_to_read);
    }
    if (!i2c_isr_err_occured)
    {
        I2C_TransferHandling(I2C1, device_addr, number_of_bytes_to_read, I2C_Generate_Start_Read, I2C_AutoEnd_Mode);
        while (index < number_of_bytes_to_read)
        {
            i2c_isr_err_occured = I2C_receive_data_wait(I2C1, &(received_data[index]));
            if (i2c_isr_err_occured)
            {
                break;
            }
        ...
        if (!i2c_isr_err_occured)
        {
            i2c_isr_err_occured = i2c_wait_flag_status(I2C1, I2C_FLAG_STOPF, SET, I2C_MAX_RETRY);
        }
    }
    return (i2c_isr_err_occured);
}
...
uint32_t i2c_write(uint16_t device_addr, uint8_t register_to_write, uint8_t data_to_write)
{
    ...
    i2c_isr_err_occured              = 0;
    I2C_TransferHandling(I2C1, device_addr, 2, I2C_Generate_Start_Write, I2C_AutoEnd_Mode);
    i2c_isr_err_occured |= i2c_error_occur(I2C1);
    i2c_isr_err_occured = I2C_send_data_wait_and_check(I2C1, register_to_write);
    i2c_isr_err_occured = I2C_send_data_wait_and_check(I2C1, data_to_write);
    i2c_isr_err_occured |= i2c_wait_flag_status(I2C1, I2C_FLAG_STOPF, SET, I2C_MAX_RETRY);
    return (i2c_isr_err_occured);
}
